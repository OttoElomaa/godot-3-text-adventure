...
func _ready():
	...
	$Hallway1.connect_exit_normal("south", $SwitchRoom, WoodenDoor)
	$Hallway1.connect_exit_locked("east", $Shrine1, MetalDoor)
	
	$Shrine1.connect_exit_normal("south", $Cave1, Passage)
	...
	var key2 = DataScene.get_file_reader().get_item_stats_by_id("key")
